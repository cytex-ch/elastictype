...
describe("Metadata", () => {
  ...
  test("when syncing, removes previous index if it exists, then creates a new index with the schema's mapping", async () => {
    ...
    expect(
      mapping2[indexName as string]?.mappings?.properties?.["name"].type
    ).toBe("keyword");
    expect(
      mapping2[indexName as string]?.mappings?.properties?.["age"].type
    ).toBe("integer");
  });
  ...
  test("should add property metadata", () => {
    const properties = TypeMetadataStorage.getPropertiesForTarget(Cat);
    expect(properties).toBeDefined();
    expect(properties.length).toBe(2);

    expect(properties[0].propertyKey).toBe("name");
    expect((properties[0].options as any).type).toBe("keyword");

    expect(properties[1].propertyKey).toBe("age");
    expect((properties[1].options as any).type).toBe("integer");
  });
  ...
  test("can execute sql query", async () => {
    ...
    Cat.sql(`SELECT * FROM cats WHERE name = '${savedCat.name}'`).then(
      (res) => {
